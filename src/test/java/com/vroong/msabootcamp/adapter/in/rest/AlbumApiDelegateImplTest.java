...
class AlbumApiDelegateImplTest {
  ...
  @MockBean private AlbumService service;

  @BeforeEach
  void setUp() {
    this.mvc =
        MockMvcBuilders.standaloneSetup(new AlbumApiController(apiDelegate))
  ...
  void createAlbum() throws Exception {
    when(service.createAlbum(any(Album.class))).thenReturn(anAlbum());
  ...
  void listAlbums() throws Exception {
    final Page<Album> pageEntityList = new PageImpl<>(List.of(anAlbum()), PageRequest.of(0, 1), 1L);
    when(service.listAlbums(any(Pageable.class))).thenReturn(pageEntityList);

    final ResultActions res =
        mvc.perform(
                get("/api/albums").contentType(Constants.V1_MEDIA_TYPE).characterEncoding("utf-8"))
            .andDo(print());
  ...
  void addSong() throws Exception {
    doNothing().when(service).addSong(anyLong(), any(Song.class));
  ...
  void removeSong() throws Exception {
    doNothing().when(service).removeSong(anyLong(), anyLong());
